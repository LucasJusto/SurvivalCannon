...
class Barrel: Enemy {
    ...
    override init(nameEnemy: String, typeEnemy: String, breakable: Bool) {
        ...
        node.size = CGSize(width: UIScreen.main.bounds.width * 0.15, height: UIScreen.main.bounds.height * 0.2)
...
class Anvil: Enemy {
    ...
    override init(nameEnemy: String, typeEnemy: String, breakable: Bool) {
        ...
        node = SKSpriteNode(texture: texture)
        node.size = CGSize(width: UIScreen.main.bounds.width * 0.15, height: UIScreen.main.bounds.height * 0.1)
        node.physicsBody = SKPhysicsBody( texture: node.texture!, size: node.texture!.size())
        super.init(nameEnemy: nameEnemy, typeEnemy: typeEnemy, breakable: breakable)
    }
}
